...
export function ContractsView() {
  ...
  if (!contract) {
    return (
      <div className="space-y-8">
        <div>
          <h2 className="font-display text-2xl font-bold text-foreground">My Contracts</h2>
          <p className="mt-1 text-sm text-muted-foreground">Select a contract to view details and traceability</p>
        </div>
          ...
          {contracts.map((c) => (
            ...
            >
              <div>
                ...
                <p className="text-sm text-muted-foreground">{c.volumeKg.toLocaleString()} kg · {c.matchedCooperative?.name || 'Unmatched'}</p>
              </div>
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => selectContract(null)} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> Back to contracts
      </button>

      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-2xl font-bold text-foreground">{contract.crop}</h2>
          <p className="text-sm text-muted-foreground">
            {contract.volumeKg.toLocaleString()} kg · {contract.matchedCooperative?.name || 'Unmatched'}
          </p>
        </div>
        {contract.escrowAmount === 0 && contract.status !== 'open' && (
          <Button
            onClick={() => { setEscrowModal(true); setEscrowSuccess(false); }}
      ...
      {contract.matchedCooperative && (
        <Card>
          <CardHeader><CardTitle className="text-base">Cooperative Details</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div><span className="text-muted-foreground">Name:</span> {contract.matchedCooperative.name}</div>
              <div><span className="text-muted-foreground">Region:</span> {contract.matchedCooperative.region}</div>
              <div><span className="text-muted-foreground">Total Hectares:</span> {contract.matchedCooperative.totalHectares}</div>
              <div><span className="text-muted-foreground">Members:</span> {contract.matchedCooperative.members.length}</div>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          {!escrowSuccess ? (
            <>
              ...
              <div className="space-y-3 py-4">
                <p className="text-sm text-muted-foreground">
                  You are about to lock <span className="font-semibold text-foreground">₱{(contract.volumeKg * 30).toLocaleString()}</span> in a secure escrow account for this contract.
                </p>
                <p className="text-xs text-muted-foreground">Funds will be released to the cooperative upon verified delivery.</p>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setEscrowModal(false)}>Cancel</Button>
                <Button onClick={handleFundEscrow} className="bg-terracotta text-terracotta-foreground hover:bg-terracotta/90">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center gap-4 py-6">
              ...
              <div className="text-center">
                <p className="font-display text-lg font-bold text-foreground">Funds Locked Successfully</p>
                <p className="mt-1 text-sm text-muted-foreground">₱{contract.escrowAmount.toLocaleString()} is now in escrow</p>
              </div>
              <Button onClick={() => setEscrowModal(false)} className="mt-2">Close</Button>
            </motion.div>
          )}
        </DialogContent>
      </Dialog>
