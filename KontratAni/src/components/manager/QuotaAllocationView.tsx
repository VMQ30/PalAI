...
export function QuotaAllocationView() {
  ...
  const progressPct = Math.min(
    (totalAllocated / Math.max(targetKg, 1)) * 100,
    100,
  );
        ...
        isNaN(parsed)
          ...
          : Math.max(
              ...
              Math.min(
                Math.round(parsed),
                Math.floor(
                  (farmers.find((f) => f.id === farmerId)?.hectares ?? 0) *
                    KG_PER_HA,
                ),
              ),
            ),
  ...
  const handleConfirm = () => {
    if (remaining < 0) {
      toast.error(
        `Over-allocated by ${Math.abs(remaining).toLocaleString()} kg. Reduce assignments first.`,
      );
      return;
    }
    if (remaining > 0) {
      toast.error(
        `${remaining.toLocaleString()} kg still unassigned. Distribute all quota before confirming.`,
      );
      return;
    }
    setIsConfirmed(true);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Distribute production quotas fairly across co-op members.
          </p>
        </div>
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="mt-3 font-medium text-muted-foreground">
              No accepted contracts yet
            </p>
            <p className="text-sm text-muted-foreground/60">
              Accept a contract from the inbox first.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl border border-border bg-muted/40 p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground leading-relaxed">
                Split by hectares. More land = can produce more = earns a larger
                share of this pool.
              </p>
              <p className="mt-2 font-mono text-sm font-bold text-primary">
                {(targetKg - basePool).toLocaleString()} kg{" "}
                <span className="font-normal text-muted-foreground">
                  ÷ {totalHa} ha
                </span>
              </p>
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between gap-6">
                ...
                <div>
                  ...
                  <p
                    className={`font-display text-xl font-bold ${remaining < 0 ? "text-destructive" : "text-primary"}`}
                  >
          ...
          <div className="space-y-3">
            {farmers.map((farmer) => {
              const maxKg = Math.floor(farmer.hectares * KG_PER_HA);
              const bd = breakdowns[farmer.id] ?? {
                base: 0,
                bonus: 0,
                total: 0,
              };
              const capPct =
                maxKg > 0 ? Math.round((bd.total / maxKg) * 100) : 0;
              const landSharePct =
                totalHa > 0
                  ? ((farmer.hectares / totalHa) * 100).toFixed(1)
                  : "0";
              const lockedPct =
                maxKg > 0 ? Math.round((bd.base / maxKg) * 100) : 0;
                      ...
                      <div className="shrink-0 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <input
                            ...
                            step={1}
                            onChange={(e) =>
                              handleTextChange(farmer.id, e.target.value)
                            }
                            onFocus={() => {
                    ...
                    <div className="mx-4 mb-3 flex items-center divide-x divide-border overflow-hidden rounded-lg border border-border text-center text-xs">
                      <div className="flex-1 bg-muted/30 px-3 py-2">
                        ...
                        <p className="mt-0.5 font-mono font-semibold text-foreground">
                          {bd.base.toLocaleString()} kg
                        </p>
                        <p className="text-muted-foreground/70">
                          locked · equal for all
                        </p>
                      </div>
                      <div className="flex-1 px-3 py-2">
                        <p className="text-muted-foreground">Pool B bonus</p>
                        <p className="mt-0.5 font-mono font-semibold text-foreground">
                          +{bd.bonus.toLocaleString()} kg
                        </p>
                        <p className="text-muted-foreground/70">
                          {landSharePct}% land share
                        </p>
                      </div>
          ...
          {remaining !== 0 && (
            ...
            >
              {remaining < 0 ? (
                ...
                <span className="h-4 w-4 shrink-0 text-center font-bold leading-4">
                  !
                </span>
              )}
              <span>
                {remaining < 0 ? (
                  <>
                    <strong>{Math.abs(remaining).toLocaleString()} kg</strong>{" "}
                    over-allocated — reduce values or reset.
                  </>
                ) : (
                  <>
                    <strong>{remaining.toLocaleString()} kg</strong> still
                    unassigned — distribute all quota to confirm.
                  </>
                )}
              </span>
            </div>
