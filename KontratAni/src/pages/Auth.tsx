          ...
          >
            {step === 1 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4 my-auto max-w-sm mx-auto w-full">
                ...
                <button onClick={() => { setRole('buyer'); setStep(2); }} className="w-full flex items-center p-4 border border-border rounded-xl hover:border-primary hover:bg-primary/5 transition-all text-left bg-background">
                  <div className="bg-primary/10 p-3 rounded-full mr-4"><Building2 className="w-6 h-6 text-primary" /></div>
                  <div>
                    <h3 className="font-semibold">Institutional Buyer</h3>
                    <p className="text-xs text-muted-foreground">Focus on Legality & Volume</p>
                  </div>
                </button>

                <button onClick={() => { setRole('coop'); setStep(2); }} className="w-full flex items-center p-4 border border-border rounded-xl hover:border-[#1c3c2b] hover:bg-accent transition-all text-left bg-background">
                  <div className="bg-[#1c3c2b]/10 p-3 rounded-full mr-4"><Users className="w-6 h-6 text-[#1c3c2b]" /></div>
                  <div>
                    <h3 className="font-semibold">Cooperative Manager</h3>
                    <p className="text-xs text-muted-foreground">Focus on Logistics & Network</p>
                  </div>
                </button>

                <button onClick={() => { setRole('farmer'); setStep(2); }} className="w-full flex items-center p-4 border border-border rounded-xl hover:border-[#8c4a2a] hover:bg-accent transition-all text-left bg-background">
                  <div className="bg-[#8c4a2a]/10 p-3 rounded-full mr-4"><Sprout className="w-6 h-6 text-[#8c4a2a]" /></div>
                  <div>
                    <h3 className="font-semibold">Solo Farmer</h3>
                    <p className="text-xs text-muted-foreground">Focus on Land & Production</p>
                  </div>
                </button>
              </motion.div>
            )}
                ...
                <div className="flex-1 overflow-y-auto pr-4 pl-2 -ml-2 pb-4 scrollbar-thin">
                  <form onSubmit={handleSignupSubmit} className="space-y-6">
                    ...
                    {role === 'buyer' && (
                      <>
                        <div className="space-y-3">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Company Details</h3>
                          <input required type="text" placeholder="Company Name (e.g. Jollibee Corp)" className={inputClass} />
                          <input required type="email" placeholder="Company Email (e.g. purchasing@jollibee.ph)" className={inputClass} />
                          <input required type="password" placeholder="Create Password" className={inputClass} />
                        </div>
                        <div className="space-y-3">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Legal & Logistics</h3>
                          <input required type="text" placeholder="Business TIN / Registration No." className={inputClass} />
                          <textarea required placeholder="Company Address" rows={2} className={`${inputClass} resize-none`} />
                    ...
                    {role === 'coop' && (
                      <>
                        <div className="space-y-3">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Cooperative Details</h3>
                          <input required type="text" placeholder="Cooperative Name (e.g. La Trinidad Assoc.)" className={inputClass} />
                          <input required type="text" placeholder="CDA / SEC Registration Number" className={inputClass} />
                          ...
                          <textarea required placeholder="Main Office Address" rows={2} className={`${inputClass} resize-none`} />
                          
                          <div className="bg-accent/50 p-3 rounded-xl border border-border">
                            <label className="text-xs text-muted-foreground block mb-1">Initial Member Count</label>
                            <input required type="number" min="1" placeholder="e.g. 50" className={inputClass} />
                            <p className="text-[10px] text-muted-foreground mt-2 flex items-center"><Info className="w-3 h-3 mr-1"/> You can add or remove members later.</p>
                          </div>
                        </div>

                        <div className="space-y-3 pt-2 border-t border-border">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">First Farmland Profiling</h3>
                          <p className="text-xs text-muted-foreground pb-2">Register your primary location. You can add more farmlands in your dashboard.</p>
                          
                          <div className="grid grid-cols-2 gap-3">
                            
                            <div className="relative">
                              <input required type="number" step="0.1" placeholder="Size" className={`${inputClass} pr-20`} />
                    ...
                    {role === 'farmer' && (
                      <>
                        <div className="space-y-3">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Personal Details</h3>
                        ...
                        <div className="space-y-3 pt-2 border-t border-border">
                          <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider">Farm Profiling</h3>
                          <input type="text" placeholder="Farm Name (Optional, e.g. Mang Juan's Acre)" className={inputClass} />
                          
                          <div className="grid grid-cols-2 gap-3">
                            
                            <div className="relative">
                              <input required type="number" step="0.1" placeholder="Farm Size" className={`${inputClass} pr-20`} />
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center border-l border-border/70 pl-2">
                                <select className="bg-transparent text-sm font-medium text-muted-foreground focus:outline-none cursor-pointer appearance-none pr-5 py-1">
                                  <option value="ha">ha</option>
                                  <option value="sqm">sqm</option>
                    ...
                    <div className="pt-4 sticky bottom-0 bg-card pb-2">
                      <button type="submit" disabled={isLoading} className="w-full py-4 rounded-xl bg-primary text-primary-foreground font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center">
                        {isLoading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Setting up Workspace...</> : "CREATE ACCOUNT"}
                      </button>
                    </div>
        ...
        <div className="absolute top-0 right-0 w-1/2 h-full p-10 flex flex-col justify-center bg-card">
          ...
          >
            <div className="text-center mb-10">
              ...
              <p className="text-muted-foreground text-sm">Login to your account to continue</p>
            </div>

            <form onSubmit={handleLogin} className="space-y-5">
              <input type="email" placeholder="Email" required disabled={isLoading} value={email} onChange={(e) => setEmail(e.target.value)} className={inputClass} />
              <input type="password" placeholder="Password" required disabled={isLoading} value={password} onChange={(e) => setPassword(e.target.value)} className={inputClass} />
              
              <div className="flex justify-center text-xs mt-2">
                <a href="#" className="text-muted-foreground hover:text-primary transition-colors font-medium">Forgot your password?</a>
              </div>
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {isLogin ? (
              ...
              <motion.div key="signup-overlay" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col items-center">
                <h2 className="text-4xl font-bold mb-4 tracking-tight">Welcome Back!</h2>
                <p className="text-primary-foreground/90 mb-8 px-4 leading-relaxed font-medium">
                  To stay connected with your network, please login with your personal info.
                </p>
                <button onClick={toggleMode} className="px-12 py-3 border-2 border-primary-foreground rounded-full font-bold tracking-wider hover:bg-primary-foreground hover:text-primary transition-all duration-300 shadow-lg">
                  SIGN IN
                </button>
