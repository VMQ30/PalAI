...
function toFarmerMember(f: Farmer): FarmerMember {
  return {
    ...
    name: f.name,
    ha: f.hectares,
    barangay: (f.region || "").replace(/^Brgy\.\s*/i, ""),
...
function TagEditor({
  ...
  const styles = {
    teal: {
      pill: "border-[#9FE1CB] bg-[#E1F5EE] text-[#085041]",
      xBtn: "hover:text-[#085041]",
    },
    green: {
      pill: "border-[#C0DD97] bg-[#EAF3DE] text-[#27500A]",
      xBtn: "hover:text-[#27500A]",
    },
    gray: {
      pill: "border-border bg-accent text-muted-foreground",
      xBtn: "hover:text-foreground",
    },
  };
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {tags.map((tag) => (
        <span
          key={tag}
          className={`flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${s.pill}`}
        >
          {tag}
          <button
            onClick={() => onChange(tags.filter((t) => t !== tag))}
            className={`ml-0.5 text-current opacity-60 ${s.xBtn}`}
          >
...
// ── Main component ────────────────────────────────────────────────────────────
export function ProfileLandView({
  isSolo,
  setIsSolo,
  coopId,
}: ProfileLandViewProps) {
  // ── Store ──────────────────────────────────────────────────────────────────
  const cooperative = useAppStore((s) =>
    s.cooperatives.find((c) => c.id === coopId),
  );
  ...
  const storeMembers: FarmerMember[] = (cooperative?.members ?? []).map(
    toFarmerMember,
  );

  // Local extra fields not in the store (crops tags, phone) — keyed by farmer id
  const [localExtras, setLocalExtras] = useState<
    Record<string, { crops: string[]; phone: string }>
  >(() =>
    Object.fromEntries(
      storeMembers.map((f) => [f.id, { crops: f.crops, phone: f.phone }]),
    ),
  );
  ...
  const [newFarmer, setNewFarmer] = useState({
    ...
    barangay: "",
    phone: "",
    paymentMode: "cash" as "ewallet" | "cash",
  });
  ...
  const addFarmer = () => {
    ...
    const storeFarmer: Farmer = {
      ...
      region: newFarmer.barangay ? `Brgy. ${newFarmer.barangay}` : "",
      lat: 0,
      lng: 0,
      soilType: "",
      smsStatus: "pending",
          ...
          <CardContent>
            <Field
              ...
              value={
                isSolo
                  ? profile.soloName
                  : (cooperative?.name ?? profile.coopName)
              }
              onChange={(v) =>
                setProfile((p) =>
                  isSolo ? { ...p, soloName: v } : { ...p, coopName: v },
                )
              }
              active={profileEdit}
            />
            ...
            <div className="mb-3.5">
              ...
              {profileEdit ? (
                <TagEditor
                  ...
                  onChange={(tags) =>
                    setProfile((p) => ({ ...p, certifications: tags }))
                  }
                  color="teal"
                  ...
                  {profile.certifications.length > 0 ? (
                    profile.certifications.map((c) => (
                      <Badge
                        key={c}
                        variant="outline"
                        className="border-[#9FE1CB] bg-[#E1F5EE] text-[10px] text-[#085041]"
                      >
                        {c}
                      </Badge>
                    ))
              ...
              <div className="rounded-lg bg-accent p-3">
                ...
                {landEdit && isSolo ? (
                  <input
                    ...
                    value={profile.plots}
                    onChange={(e) =>
                      setProfile((p) => ({ ...p, plots: +e.target.value }))
                    }
                  ...
                  <div className="mt-1">
                    <p className="text-xl font-semibold text-foreground">
                      {totalCoopHa.toFixed(1)} ha
                    </p>
                    <p className="mt-0.5 text-[10px] text-muted-foreground">
                      Auto-summed from members
                    </p>
                  </div>
                ) : (
            ...
            <div>
              ...
              {landEdit ? (
                <TagEditor
                  ...
                  onChange={(tags) =>
                    setProfile((p) => ({ ...p, primaryCrops: tags }))
                  }
                  color="green"
                  ...
                  {profile.primaryCrops.length > 0 ? (
                    profile.primaryCrops.map((c) => (
                      <Badge
                        key={c}
                        variant="outline"
                        className="border-[#C0DD97] bg-[#EAF3DE] text-[10px] text-[#27500A]"
                      >
                        {c}
                      </Badge>
                    ))
                      ...
                      <div className="rounded-lg border border-dashed border-border p-3">
                        ...
                        <div className="mb-3">
                          ...
                          {phoneErr ? (
                            ...
                            <p className="mt-1 text-[10px] font-medium text-[#0F6E56]">
        ...
        {isSolo && (
          ...
          >
            ...
            <p className="text-sm leading-relaxed text-[#085041]">
              <span className="font-semibold">Solo Farmer mode active.</span>{" "}
              The member farmers database is bypassed. You are automatically set
              as the sole fulfiller for all accepted contracts.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
